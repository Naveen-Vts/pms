...
//	private static final String ACTIONWISEALLREPORT="CALL Pfms_Action_Wise_All_Reports (:term,:empid,:projectid);";
	...
	private static final String ALLEMPNAMEDESIGLIST="SELECT e.empid ,CONCAT(IFNULL(CONCAT(e.title,' '),''), e.emp_name) as 'empname' , ed.designation FROM employee e, employee_desig ed WHERE e.desig_id=ed.desig_id";
...
//		int result=query.executeUpdate();
	...
	private static final String GETACTIONFORFEVORITE="SELECT a.actionmainid,CONCAT(IFNULL(CONCAT(b.title,' '),''), b.emp_name) AS 'assignorempname',c.designation,a.actiondate,d.enddate,a.actionitem,d.actionstatus,d.actionstatus as 'status',d.remarks,d.actionno ,d.actionassignid ,d.assignee ,d.assignor ,a.actionlevel ,(SELECT c.progress FROM action_sub c  WHERE  c.actionassignid = d.actionassignid AND c.actionsubid =(SELECT MAX(b.actionsubid) FROM action_sub b WHERE b.actionassignid = d.actionassignid))  AS progress ,d.pdcorg, CONCAT(IFNULL(CONCAT(e.title,' '),''), e.emp_name) AS 'assigneeempname' , a.type FROM  action_main a, employee b ,employee_desig c , action_assign d , employee e WHERE a.actionmainid=d.actionmainid AND d.assignor=b.emp_id AND b.is_active='1' AND d.assignee=e.emp_id AND c.desig_id=b.desig_id   AND d.actionstatus NOT IN ('C') AND d.actionassignid NOT IN (SELECT actionassignid FROM Pfms_Favourite_List WHERE empid=:empid) AND c.desigsr <=(SELECT ed.desigsr FROM employee e, employee_desig ed WHERE e.desigid=ed.desigid AND e.emp_id=:empid) AND a.projectid=:projectid AND a.actiondate BETWEEN :fromdate AND :todate ORDER BY d.actionassignid DESC";
	@Override
