...
public class ReportDaoImpl implements ReportDao {
	...
	private static final String prjDetails="SELECT  m.project_category, m.project_short_name,  m.project_name, m.project_description, m.sanction_no,  m.sanction_date, ROUND(m.total_sanction_cost / 100000, 2) AS 'TotalSanctionCost',   ROUND(m.sanction_cost_re / 100000, 2) AS 'SanctionCostRE',   ROUND(m.sanction_cost_fe / 100000, 2) AS 'SanctionCostFE', m.Objective,  m.lab_participating, m.scope,  c.category, s.ProjectStageCode,s.ProjectStage, (SELECT COUNT(ScheduleId) FROM committee_schedule WHERE CommitteeId='1' AND ProjectId=:projectid AND  ScheduleFlag IN ('MKV','MMR','MMF','MMS','MMA')) AS PMRB,  (SELECT COUNT(ScheduleId) FROM committee_schedule WHERE CommitteeId='2' AND ProjectId=:projectid AND  ScheduleFlag IN ('MKV','MMR','MMF','MMS','MMA')) AS EB ,p.Brief,p.ImageName,t.project_type,CONCAT(p.Path,p.ImageName)AS imgpath FROM  project_master m LEFT JOIN  pfms_category c ON c.categoryid = m.project_category LEFT JOIN  pfms_project_data d ON d.ProjectId = m.project_id LEFT JOIN  pfms_project_stage s ON s.ProjectStageId = d.CurrentStageId LEFT JOIN pfms_project_slides p ON p.ProjectId = m.project_id LEFT JOIN  project_type t ON t.project_type_id = m.project_type  WHERE m.project_id = :projectid";
	@Override
	...
	private static final String mileStoneData="SELECT a.project_id,b.ActivityName,b.StartDate,b.EndDate ,'2024' AS PlanActivityYear,b.DateOfCompletion,YEAR(b.DateOfCompletion) AS 'completedYear',YEAR(b.StartDate) AS 'StartYear',YEAR(b.EndDate) AS 'EndYear',b.MilestoneActivityId,b.MilestoneNo,b.ProgressStatus  FROM project_master a,milestone_activity b WHERE a.project_id=b.ProjectId AND a.is_active=1 AND b.IsActive=1 AND a.project_id=:prjid ";
	@Override
